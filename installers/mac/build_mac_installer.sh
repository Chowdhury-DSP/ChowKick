...
pkg_dir=ChowKick_Installer_Packaged
rm -Rf $pkg_dir
mkdir $pkg_dir
productsign -s "$TEAM_ID" ../../build/ChowKick.pkg $pkg_dir/ChowKick-signed.pkg

echo "Notarizing installer package..."
INSTALLER_PASS=$(more ~/Developer/mac_installer_pass)
npx notarize-cli --file $pkg_dir/ChowKick-signed.pkg --bundle-id com.chowdsp.ChowKick --asc-provider "$TEAM_ID" --username chowdsp@gmail.com --password "$INSTALLER_PASS"
